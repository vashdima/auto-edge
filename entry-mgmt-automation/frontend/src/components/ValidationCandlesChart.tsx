...
function sliceValidationUpToEntry(trade: EntryMap) {
  const entryMs = Date.parse(trade.entryTime)
  return trade.validationBuffer.filter((b) => Date.parse(b.time) <= entryMs)
}

export function ValidationCandlesChart({ trade }: { trade: EntryMap | null }) {
  ...
  const { candles, emaSlow, emaMedium, emaFast } = useMemo(() => {
    if (!trade) {
      return { candles: [] as { time: UTCTimestamp; open: number; high: number; low: number; close: number }[], emaSlow: [], emaMedium: [], emaFast: [] }
    }
    const entryMs = Date.parse(trade.entryTime)
    const buf = mode === 'upToEntry' ? sliceValidationUpToEntry(trade) : trade.validationBuffer
    const withOhlc = buf.filter((b) => b.open != null && b.high != null && b.low != null && b.close != null)
    let lastIndexBeforeEntry = -1
    withOhlc.forEach((b, idx) => {
  ...
  if (!trade) {
    return (
      <div className="chart-placeholder">
        <h3>Validation TF</h3>
        <span>Select a trade to view validation</span>
      </div>
    )
  }
  ...
  return (
    <div className="chart-panel">
      <div className="chart-panel-header">
        <h3>Validation TF (up to entry)</h3>
        <span className="chart-panel-subtitle">{trade.symbol} {trade.validationTF}</span>
        {!hasEmaData && (
          <span className="chart-panel-subtitle chart-panel-hint" title="Run scanner_indicators.py to populate EMAs in the DB">
            No validation EMA data — run pipeline: mtf_loader → scanner_indicators → scanner_entry_mgmt
          </span>
        )}
      </div>
